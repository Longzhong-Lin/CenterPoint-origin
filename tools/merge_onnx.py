...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Merge onnx")
    parser.add_argument(
    ...
    encoder_path = os.path.join(args.work_dir, "encoder_sim.onnx")
    decoder_path = os.path.join(args.work_dir, "decoder.onnx")
    model_path = os.path.join(args.work_dir, "centerpoint.onnx")
    trt_model_path = os.path.join(args.work_dir, "centerpoint_trt.onnx")
    ...
    encoder.graph.node.extend(decoder.graph.node)
    encoder.graph.output.pop()
    encoder.graph.output.extend(decoder.graph.output)
    encoder.graph.initializer.extend(decoder.graph.initializer)
    full_model = copy.deepcopy(encoder)
    trt_model = copy.deepcopy(encoder)

    # Connect pfe and rpn with scatterND
    make_scatterND(full_model, rpn_input_shape, indices_shape, pfe_out_maxpool_name, batch_size)
    make_scatterND(trt_model, rpn_input_shape, indices_shape, pfe_out_maxpool_name, batch_size, save_for_trt=True)
